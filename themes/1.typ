...
#definition[
  Пусть имеется правило, которое каждому натуральному числу $n$ ставит в соответствие некоторое $x_n$ из множества $G$.
  Тогда *последовательностью* называется множество всевозможных упорядоченных пар $(n, x_n), n in NN$.
]

#definition[
  Последовательность ${x_n}$ называется *ограниченной сверху (снизу)*, если
  #eq[
...
#definition[
  Последовательность ${y_k}$ называется *подпоследовательностью* последовательности ${x_n}$, если
  #eq[
    $forall k in NN : exists n = n_k : y_k = x_n_k$,
  ]
  где последовательность ${n_k}$ - строго возрастающая. Эта последовательность обозначается ${x_n_k}$.
]

#definition[
  Последовательность отрезков ${[a_n, b_n]}, n in NN$ называется *последовательностью вложенных отрезков*, если
  #eq[
...
#theorem(
  ...
  Всякая последовательность вложенных отрезков ${[a_n, b_n]}_(n = 1)^oo$ имеет непустое пересечение, то есть
  #eq[
...
#definition[
  Число $x_0$ называется пределом последовательности ${x_n}$, если
  #eq[

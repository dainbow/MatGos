...
#proof[
  Возьмем $0 < r < R$, тогда $f$ голоморфна в $overline(O_r (a))$. Тогда по
  теореме Коши
  $2 pi i space f(z) = integral_(gamma_r) (f(xi) d xi) / (xi - z)$.

  Распишем #eq[
    $1 / (xi - z) = 1 / ((xi - a) - (z - a)) = 1 / ((xi - a)) dot.op 1 / (1 - (z - a) / (xi - a)) =^((|z-a| < |xi - a|)) = 1 / (xi - a) sum_(n = 0)^infinity ((z-a)/(xi-a))^n = sum_(n = 0)^infinity (z-a)^n/(xi-a)^(n+1)$
  ]

  Полученный ряд сходится равномерно, а значит можно почленно интегрировать.

  $2 pi i space f(z) = integral_(gamma_r) f(xi) / (xi - z) d xi = sum_(n = 0)^infinity integral_(gamma_r) f(xi) (z-a)^n/(xi-a)^(n+1) d xi = sum_(n=0)^(infinity) 2 pi i c_n (z-a)^n$

  Причем по следтвию формулы Коши для круга, $2 pi i dot.op c_n = (f^((n)) (a)) / n!$

...
#definition[
  Пусть $f$ -- функция, $D_f$ -- ее область определения,  $x_0 in D_f$. Тогда *производной* $f$ в точке $x_0$ называется
  #eq[
...
#theorem[
  Функция $f$, определенная в некоторой окрестности точки $x_0$ имеет производную в точке $x_0$ тогда и только тогда,
  когда она имеет обе односторонние производные в этой точке, и эти производные равны.
]

#definition[
  Функция $f$ называется *дифференцируемой* в точке $x_0$, если она определена в некоторой окрестности точки $x_0$ и
  в этой точке имееет конечную производную.
]

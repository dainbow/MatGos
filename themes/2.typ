...
#definition[
  Множество $E subset RR$ называется *ограниченным*, если оно ограниченно и сверху, и снизу.
]

#definition[
  Число $M$ называется *точной верхней гранью* множества $E$ и обозначается $sup E$, если
  #eq[
    1. $forall x in E : x <= M$;
    2. $forall M' < M : exists x in E : x > M'$.
  ]
]

#definition[
  Число $m$ называется *точной нижней гранью* множества $E$ и обозначается $inf E$, если
  #eq[
    1. $forall x in E : x >= m$;
    2. $forall m' > m : exists x in E : x < m'$.
  ]
...
#definition[
  Пусть $D$ и $Y$ -- два произвольных множества, и задано некоторое правило $f$, которое каждому элементу $x in D$
  ставит в соответствие один и только один некоторый элемент $y = f(x)$ из $Y$. Тогда множество всевозможных пар
  $(x, f(x)), x in D$ называется функцией.
]

#definition[
  Пусть $f$ - функция, а $D_f$ -- ее бласть определения. Тогда $c$ называется *пределом по Коши* функции $f$ в точке
  $x_0 in D_f$, если
  #eq[
...
#definition[
  Последовательность ${x_n} subset D_f$ называется Гейне функции $f$ в точке $x_0$, если
  #eq[
    1. $forall n in NN : x_n in D_f backslash {x_0}$;
    2. $lim_(n -> oo) x_n = x_0$.
  ]
]

#definition[
  Пусть $f$ - функция, а $D_f$ -- ее бласть определения. Тогда $c$ называется *пределом по Гейне* функции $f$ в точке
  $x_0 in D_f$, если
  #eq[
    $forall {x_n}$ -- последовательность Гейне $lim_(n -> oo) f(x_n) = c$.
  ]
]
